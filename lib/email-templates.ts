  ...
  applicationApproved: (contractorName: string, companyName: string): EmailTemplate => ({
    ...
    html: layout(
      ...
      ${successCard(`
        ...
        ${cta('Sign In to Your Dashboard &rarr;', `${BASE_URL}/contractor-portal?redirect=${encodeURIComponent('/contractor-dashboard')}`)}
        ${linkFallback(`${BASE_URL}/contractor-portal?redirect=${encodeURIComponent('/contractor-dashboard')}`)}
      `)}
