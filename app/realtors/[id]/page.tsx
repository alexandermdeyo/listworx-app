...
export default async function RealtorProfilePage({
  ...
  const kit = brandKit as BrandKit;
  ...
  // ── Generate signed URLs for listing primary photos ──────────────────────────
  const listings: ListingWithPhoto[] = await Promise.all(
  ...
  const showcase = (showcaseRaw ?? []) as ShowcasePost[];
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Barlow', Arial, sans-serif" }}>
      ...
      <div
        ...
        style={kit.cover_photo_url ? { backgroundImage: `url(${kit.cover_photo_url})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}
      >
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ── Contact bar ───────────────────────────────────────────────── */}
        {(kit.phone || kit.email || kit.website || kit.instagram_handle || kit.facebook_url || kit.linkedin_url) && (
          <div className="flex flex-wrap items-center gap-3 mb-8">
            {kit.phone && (
              <a
                href={`tel:${kit.phone.replace(/[^0-9+]/g, '')}`}
            ...
            {kit.website && (
              <a
                href={kit.website.startsWith('http') ? kit.website : `https://${kit.website}`}
                target="_blank"
            ...
            {kit.instagram_handle && (
              <a
                href={`https://instagram.com/${kit.instagram_handle}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition"
              >
        ...
        {/* ── Disclaimer ────────────────────────────────────────────────── */}
        {kit.disclaimer_text && (
          <p className="text-xs text-gray-400 leading-relaxed pb-12">
            {kit.disclaimer_text}
          </p>
        )}
