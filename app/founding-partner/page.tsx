...
const pricingRows = [
  ['Locked Monthly Rate', '$199/mo', '$349/mo', '$599/mo'],
  ['Standard Rate', '$249/month', '$429/month', '$729/month'],
  ['You Save', '$50/mo forever', '$80/mo forever', '$130/mo forever'],
    ...
    <PageShell surface="marketing">
      ...
      <section className="container mx-auto px-4 py-12">
        <Reveal as="div">
          ...
          <p className="mb-6 text-mkt-ink/70">Your renewal rate depends on the tier you choose — and it&apos;s locked for life.</p>
          ...
          <div className="overflow-x-auto rounded-xl border border-zinc-200 -webkit-overflow-scrolling-touch">
            <table className="w-full min-w-[680px] bg-white text-left text-sm">
              <thead><tr>{['', 'Network Member', 'Network Partner', 'Network Elite'].map(h => <th key={h} className="p-4 text-mkt-ink whitespace-nowrap">{h}</th>)}</tr></thead>
              <tbody>{pricingRows.map(row => <tr key={row[0]} className="border-t border-zinc-200">{row.map((cell, i) => <td key={`${row[0]}-${i}`} className={`p-4 whitespace-nowrap ${i === 0 ? 'font-semibold text-mkt-ink' : 'text-mkt-ink/70'}`}>{cell}</td>)}</tr>)}</tbody>
            </table>
          </div>
