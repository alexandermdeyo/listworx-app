...
import { Loader as Loader2, CircleAlert as AlertCircle, Shield } from 'lucide-react';
...
export default function ApplyPage() {
  ...
  useEffect(() => {
    ...
    async function resolveApplyState() {
      ...
      try {
        ...
        console.log('[apply] auth resolution', {
          userId: user.id,
          role,
  ...
  const unauthenticatedGate = useMemo(
    () => (
      <div className="bg-lw-surface-card rounded-2xl border border-lw-border-light p-8 shadow-sm max-w-xl mx-auto text-center">
        ...
        <h1 className="text-3xl font-bold text-lw-text mb-2">Continue Your Contractor Application</h1>
        <p className="text-lw-text/60 mb-6">
          Sign in or create your contractor account in the contractor portal, then you&apos;ll land here to complete your application.
        </p>

        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button asChild className="bg-lw-rust hover:bg-lw-rust-hover text-white">
            <Link href="/contractor-portal?mode=signup">Create Contractor Account</Link>
          </Button>
          <Button asChild variant="outline">
            <Link href="/contractor-portal?mode=login">Sign In</Link>
          </Button>
        </div>
      </div>
    ),
